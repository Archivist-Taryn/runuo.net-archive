...
namespace Server.Guilds
{
	public class GuildMemberInfoGump : BaseGuildGump
	{
		...
		public override void PopulateGump()
		{
			...
			AddBackground( 0, 0, 350, 290, 0x242C );
			...
			//Guildmate Travel Edit
			AddBackground( 20, 240, 150, 26, 0x2486 );
			AddButton( 25, 245, 0x845, 0x846, 6, GumpButtonType.Reply, 0 );
			AddHtml( 50, 243, 110, 26, "Go to Guildmate", false, false );
			
			AddBackground( 180, 240, 150, 26, 0x2486 );
			AddButton( 185, 245, 0x845, 0x846, 7, GumpButtonType.Reply, 0 );
			AddHtml( 210, 243, 110, 26, "Bring Guildmate", false, false );
			//End Edit
		}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch( info.ButtonID )
			{
				...
				//Guildmate Travel Edit
				case 6:	//Go to guildmate
				{
					if( !( m_Member == pm ) )
					{
						pm.SendMessage( "Their permission has been requested." );
						m_Member.SendGump( new GoToGuildMatePermissionGump( m_Member, guild, pm ) );
					}
					else
						pm.SendMessage( "You are already there!" ); 

					break;
				}
				case 7:	//Bring guildmate
				{
					if( !( m_Member == pm ) )
					{
						pm.SendMessage( "Their permission has been requested." );
						m_Member.SendGump( new BringGuildMatePermissionGump( m_Member, guild, pm ) );
					}
					else
						pm.SendMessage( "They are already here!" ); 
