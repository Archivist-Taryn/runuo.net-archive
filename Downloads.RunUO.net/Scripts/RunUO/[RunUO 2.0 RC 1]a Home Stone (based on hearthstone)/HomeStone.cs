...
{ 
	...
	{ 
		private static readonly TimeSpan delay = TimeSpan.FromMinutes( 60.0 );
		private static readonly bool oneStonePerMobile = true; 
		private static readonly TimeSpan totalDelay = TimeSpan.FromMinutes( 60.0 );
		...
		private bool Validate( Mobile from )  
		{
			...
			{
				from.SendMessage( "You have to wait until the homestone is ready." ); 
				return false;
			}
			else if ( HomeStone.GetRemainingTimeToUseForMobile( from ) > TimeSpan.Zero && oneStonePerMobile )
			{
				from.SendMessage( "You have to wait {0} Minutes until you can use another homestone.", (int)HomeStone.GetRemainingTimeToUseForMobile( from ).TotalMinutes );
				return false;
			}			
			else 
			{
		...
		private class HomeStoneSpell : Spell
		{
			private static SpellInfo m_Info = new SpellInfo( "Home Stone", "", SpellCircle.Second, 230 );
...
//			Effects.SendLocationEffect( new Point3D( caster.X + 1, caster.Y + 1, caster.Z - 1 ), caster.Map, 0x3709, 30, 1281, 2 );
			...
			{
				return TimeSpan.FromSeconds(5);//( (m_Mount.IsDonationItem && RewardSystem.GetRewardLevel( m_Rider ) < 3 )? 12.5 : 5.0 ) );
			}
